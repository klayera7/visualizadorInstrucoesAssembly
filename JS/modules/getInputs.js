...
const loadSegmentsIntoRegisters = () => {
  for (const key in segmentData) {
    const selector = segmentData[key].registerSelector;
 ...
 const getSegmentValues = () => {
  ...
  confirmBtn.addEventListener("click", () => {
    if (iframeWindow) {
      for (const key in segmentData) {
        const inputId = segmentData[key].inputId;
 ...
 export const initSegments = ()=>{
   document.addEventListener('DOMContentLoaded', ()=>{
     const iframe = document.querySelector('#segment_popup');
     if (iframe) iframe.addEventListener('load', getSegmentValues)
   })
 }
